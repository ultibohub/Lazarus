...
var
  ...
  begin
    if FileVersion<3 then
      ...
      SmartLinkUnit := aXMLConfig.GetValue(p+'SmartLinkUnit/Value', True); //Ultibo
  end;
  ...
  begin
    if FileVersion<3 then
      ...
      LinkSmart := aXMLConfig.GetValue(p+'LinkSmart/Value', True); //Ultibo
  end;
...
begin
  ...
  OptimizationLevel := aXMLConfig.GetValue(p+'Optimizations/OptimizationLevel/Value', 2); //Ultibo
  ...
  { Linking }
  p:=Path+'Linking/';
  GenerateDebugInfo := aXMLConfig.GetValue(p+'Debugging/GenerateDebugInfo/Value', False); // Default = True, since version 11 (was False before) //Ultibo
  UseLineInfoUnit := aXMLConfig.GetValue(p+'Debugging/UseLineInfoUnit/Value', False); //Ultibo
